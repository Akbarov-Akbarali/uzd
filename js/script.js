const body = document.querySelector("body")
const menu_icon = document.querySelector('.menu_icon')
const menu_body = document.querySelector('.menu_body')

if (menu_icon) {
    menu_icon.addEventListener("click", () => {
        menu_icon.classList.toggle("active")
        menu_body.classList.toggle("active")
        body.classList.toggle("lock")
        header_svg.classList.remove("active")
        shop_wrapper.classList.remove("active")
    })
}

const header_svg = document.querySelector(".header_svg")
const shop_wrapper = document.querySelector(".shop_wrapper")
const shop_svg = document.querySelector(".shop_svg")

if (header_svg) {
    header_svg.addEventListener("click", () => {
        shop_wrapper.classList.toggle("active")
        body.classList.toggle("lock")
        header_svg.classList.toggle("active")
        menu_icon.classList.remove("active")
        menu_body.classList.remove("active")
    })
}
if (shop_svg) {
    shop_svg.addEventListener("click", () => {
        shop_wrapper.classList.remove("active")
        body.classList.remove("lock")
        header_svg.classList.remove("active")
    })
}

const header_burger = document.querySelector(".header_burger")
const header_burger_list = document.querySelector(".header_burger_list")
const header_burger_svg = document.querySelector(".header_burger_svg")

if (header_burger) {
    header_burger.addEventListener("click", () => {
        header_burger_list.classList.toggle("active")
        header_burger.classList.toggle("active")
        header_burger_svg.classList.toggle("active")
        header_nav_list.classList.remove("active")
        header_nav_svg.classList.remove("active")
        header_nav_btn.classList.remove("active")
        body.classList.toggle("lock")
    })
}
if (header_burger_svg) {
    header_burger_svg.addEventListener("click", () => {
        header_burger_list.classList.remove("active")
        header_burger.classList.remove("active")
        header_burger_svg.classList.remove("active")
    })
}

const header_nav_btn = document.querySelector(".header_nav_btn")
const header_nav_list = document.querySelector(".header_nav_list")
const header_nav_svg = document.querySelector(".header_nav_svg")

if (header_nav_btn) {
    header_nav_btn.addEventListener("click", () => {
        header_nav_list.classList.toggle("active")
        header_nav_svg.classList.toggle("active")
        header_nav_btn.classList.toggle("active")
        header_burger.classList.remove("active")
        header_burger_list.classList.remove("active")
        header_burger_svg.classList.remove("active")
        body.classList.toggle("lock")
    })
}
if (header_nav_svg) {
    header_nav_svg.addEventListener("click", () => {
        header_nav_list.classList.remove("active")
        header_nav_svg.classList.remove("active")
        header_nav_btn.classList.remove("active")
    })
}